...
for video in data:
    ...
    if not os.path.exists(file_path_desc):
    ...
    if not utils.terminated_requested and not os.path.exists(file_path_composite):
        ...
        # hardware encoding: -hwaccel auto -loglevel quiet
        # hardware encoding: -c:v h264_nvenc -preset:v medium -profile:v main
        # hardware encoding: -vcodec libx264 -crf 20 -preset veryfast
        t0 = time.time()
        if os.path.exists(file_path_render):
            times_start = video["t_start"].split(",")
            times_end = video["t_end"].split(",")
            assert len(times_start) == len(times_end)
            print("\t- rendering with chat overlay ("+str(len(times_start))+" segments)")


            cmd = path_twitch_ffmpeg + ' -loglevel quiet -hide_banner -stats ' \
