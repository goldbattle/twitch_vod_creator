...
if [ -d "$LIBTENSORFLOW_DIR" ] && [ -d "$LIBTENSORFLOW_DIR/lib" ] && [ -d "$LIBTENSORFLOW_DIR/include" ]; then
    ...
    LIBTENSORFLOW_URL="https://storage.googleapis.com/tensorflow/versions/${VERSION_LIBTENSORFLOW}/libtensorflow-gpu-linux-x86_64.tar.gz"

    # Try GPU version first, fall back to CPU if it fails
    if ! curl -fsSL "$LIBTENSORFLOW_URL" | tar -xzC "$LIBTENSORFLOW_DIR" -f -; then
        echo "GPU version not available, trying CPU version..."
        LIBTENSORFLOW_URL="https://storage.googleapis.com/tensorflow/versions/${VERSION_LIBTENSORFLOW}/libtensorflow-cpu-linux-x86_64.tar.gz"
...
# Add common CUDA installation locations (check if they exist first)
...
[ -d "/usr/lib" ] && export LD_LIBRARY_PATH="/usr/lib:${LD_LIBRARY_PATH}"
[ -d "/usr/lib64" ] && export LD_LIBRARY_PATH="/usr/lib64:${LD_LIBRARY_PATH}"

# Add cuDNN paths if found (cuDNN is usually in the same directory as CUDA)
# But check common separate locations too
